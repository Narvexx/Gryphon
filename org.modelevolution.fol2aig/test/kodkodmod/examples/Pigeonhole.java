...
public final class Pigeonhole {
	...
	public static void main(String[] args) {
		...
		try {
			...
			final Formula show = model.declarations()
					.and(model.pigeonPerHole());
			final Solution sol = solver.solve(show, model.bounds(p, h));
			// System.out.println(show);
			System.out.println(sol);
			if (sol.unsat()) {
				System.out.println("UNSAT");
				System.out.println(sol.proof().log().roots().toString());
				for (Iterator<TranslationRecord> it = sol.proof().core(); it
						.hasNext();) {
					TranslationRecord r = (TranslationRecord) it.next();
					System.out.println(r);
				}
				//sol.proof().minimize(Re);
